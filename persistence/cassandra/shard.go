...
var _ shard.Storage = new(cassandraShardStorage)

type cassandraShardStorage struct {
	shardId string
	*baseDao
	encoderDecoder util.EncoderDecoder[model.FlowContext]
}

func NewCassandraStorage(baseDao *baseDao, encoderDecoder util.EncoderDecoder[model.FlowContext], shardId string) *cassandraShardStorage {
	return &cassandraShardStorage{
		baseDao:        baseDao,
...
func (r *cassandraShardStorage) SaveFlowContext(wfName string, flowId string, flowCtx *model.FlowContext) error {
...
func (r *cassandraShardStorage) GetFlowContext(wfName string, flowId string) (*model.FlowContext, error) {
	key := r.baseDao.getNamespaceKey(WORKFLOW_KEY, wfName, r.shardId)
...
func (r *cassandraShardStorage) DeleteFlowContext(wfName string, flowId string) error {
	key := r.baseDao.getNamespaceKey(WORKFLOW_KEY, wfName, r.shardId)
	ctx := context.Background()
...
func (r *cassandraShardStorage) SaveFlowContextAndDispatchAction(wfName string, flowId string, flowCtx *model.FlowContext, actions []model.ActionExecutionRequest) error {
...
func (r *cassandraShardStorage) PollAction(actionType string, batchSize int) ([]model.ActionExecutionRequest, error) {
...
func (r *cassandraShardStorage) Ack(actionType string, actions []model.ActionExecutionRequest) error {
	queueName := r.getNamespaceKey(actionType, r.shardId)
	ctx := context.Background()
	var messages []string
...
func (r *cassandraShardStorage) Retry(wfName string, flowId string, actionName string, actionId int, delay time.Duration) error {
...
func (r *cassandraShardStorage) PollRetry() ([]model.ActionExecutionRequest, error) {
	queueName := r.getNamespaceKey("retry", r.shardId)
...
func (r *cassandraShardStorage) Delay(wfName string, flowId string, actionName string, actionId int, delay time.Duration) error {
	message := fmt.Sprintf("%s:%s:%s:%d", wfName, flowId, actionName, actionId)
	queueName := r.getNamespaceKey("delay", r.shardId)
	return r.addToSortedSet(queueName, []string{message}, delay)
}
func (r *cassandraShardStorage) PollDelay() ([]model.ActionExecutionRequest, error) {
	queueName := r.getNamespaceKey("delay", r.shardId)
	values, err := r.getExpiredFromSortedSet(queueName)
	if err != nil {
		return nil, err
...
func (r *cassandraShardStorage) Timeout(wfName string, flowId string, actionName string, actionId int, delay time.Duration) error {
	message := fmt.Sprintf("%s:%s:%s:%d", wfName, flowId, actionName, actionId)
	queueName := r.getNamespaceKey("timeout", r.shardId)
	return r.addToSortedSet(queueName, []string{message}, delay)
}

func (r *cassandraShardStorage) PollTimeout() ([]model.ActionExecutionRequest, error) {
	queueName := r.getNamespaceKey("timeout", r.shardId)
	values, err := r.getExpiredFromSortedSet(queueName)
	if err != nil {
		return nil, err
...
func (r *cassandraShardStorage) addToSortedSet(key string, messages []string, delay time.Duration) error {
	ctx := context.Background()
...
func (r *cassandraShardStorage) getExpiredFromSortedSet(key string) ([]string, error) {
	ctx := context.Background()
